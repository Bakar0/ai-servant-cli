...
// Walking a Summons's scope. Shared by the startup tree and the agent's own grep so both see
...
export const SUMMONS_IGNORED_DIRS: ReadonlySet<string> = new Set([
  ".git",
  "node_modules",
  "dist",
  "out",
...
export async function walkScopeFiles(root: string, limits: WalkLimits = {}): Promise<string[]> {
  ...
  async function walk(dir: string, prefix: string, depth: number): Promise<void> {
    ...
    for (const entry of entries.toSorted((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (SUMMONS_IGNORED_DIRS.has(entry.name)) continue;
